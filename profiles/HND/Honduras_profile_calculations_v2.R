    ...
    if(length(yrs) < 11){
      ...
      if(length(yrs) == 1){
        ...
        for(j in 1:nrow(tmsr)){
          ...
          if(i != 20){tmsr[j,] <- dff} else {tmsr[j,] <- dff/5}
          tmsr$rate_of_change[j] <- mean(x = as.numeric(tmsr[j,]), na.rm = T)
          if(i != 20){tmsr$rate_of_change_cleaned[j] <- mean_corrected(dff = dff)} else {tmsr$rate_of_change_cleaned[j] <- tmsr$rate_of_change[j]}
        }; rm(j)
        tmsr <- round(tmsr * 100, 2)
      }
      ...
      tmsr <- df[,grep2(pattern = ini:end, x = names(df))] # 103 KEN
