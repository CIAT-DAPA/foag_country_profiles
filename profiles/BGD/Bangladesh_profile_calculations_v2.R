...
# Summary table
smy <- readxl::read_excel(path = paste0(root,'/2021/profiles/BGD/data_revised/Bangladesh_indicators.xlsx'), sheet = 'Resume')
...
tbls <- 1:30 %>%
  purrr::map(.f = function(i){
    ...
    df <- readxl::read_excel(path = paste0(root,'/2021/profiles/BGD/data_revised/Bangladesh_indicators.xlsx'), sheet = sheet)
    ...
    col_id <- which(apply(X = base::as.data.frame(df[which(df$ISO == 'BGD'),]), MARGIN = 2, FUN = function(x){!is.na(x)}))
...
out <- paste0(root,'/2021/profiles/BGD/results/BGD_calculations_v2.xlsx')
