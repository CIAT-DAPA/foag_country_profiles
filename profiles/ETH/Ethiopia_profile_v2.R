...
sts <- 1:30 %>%
  purrr::map(.f = function(i){
    ...
    rch <- readxl::read_excel(path = paste0(root,'/2021/profiles/ETH/results/ETH_calculations_v2.xlsx'), sheet = i)
    ...
    sts$Indicator <- rch %>% base::as.data.frame() %>% .[,paste0('Indicator',i)] %>% unique()
